...
select * from movie 
where (id,title,year,date_published,duration,country,worlwide_gross_income,languages,production_company)
 is null;
...
-- Type your code below:
select genre , count(title) movie_count
from genre g
inner join movie m
on g.movie_id = m.id
group by genre
...
select title, round(avg(avg_rating) ,1) as avg_rating ,
...
select genre , count(movie_id) as movie_count -- ,total_votes  , country , month(date_published)
from movie m 
inner join genre g
on g.movie_id = m.id
inner join ratings r
using(movie_id)
where m.year=2017 and month(date_published)=3
and country='USA' 
and total_votes>1000
group by genre -- ,  month(date_published)
order by movie_count desc;
...
+---------------+-------------------+---------------------+*/
-- Type your code below:
select title , round(avg_rating ,1) as avg_rating , genre
from movie m
inner join ratings r 
on m.id = r.movie_id
inner join genre
...
select  count(date_published ) as count_of_movies
from movie m
inner join ratings r
on m.id = r.movie_id
...
select * from top_production where prod_comp_rank<4;
...
select name , (total_votes)  -- as total_votes
 from names n 
 inner join movie m
 using(id)
 inner join ratings r
